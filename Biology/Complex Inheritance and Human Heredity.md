...
**核型 (Karyotype)** 是有丝分裂中期染色体的显微照片——将同源染色体按大小递减排列。人类核型包含 **23 对染色体**：**22 对常染色体 (Autosome)** + **1 对性染色体 (Sex Chromosome)**。

核型分析可以检测染色体数目的异常和较大的结构变异，是遗传病诊断的重要工具。

### 5.2 染色体不分离 (Nondisjunction)

在减数分裂过程中，**姐妹染色单体 (Sister Chromatids)** 应当分别移向细胞两极，使每个子细胞获得正确数量的染色体。但偶尔会发生**不分离 (Nondisjunction)**——染色单体未能正确分开。

```mermaid
graph TB
    subgraph 正常减数分裂
        A["正常分离"] --> B["每个配子<br/>1 条染色体"]
    end
    
    subgraph 不分离（减数分裂 I）
        C["同源染色体<br/>不分离"] --> D["2 个配子：多一条"]
        C --> E["2 个配子：少一条"]
    end
    
    subgraph 不分离（减数分裂 II）
        F["姐妹染色单体<br/>不分离"] --> G["1 个配子：多一条"]
        F --> H["1 个配子：少一条"]
        F --> I["2 个配子：正常"]
    end
...
当不分离产生的配子与正常配子受精后，子代会具有异常的染色体数目：

| 异常类型 | 定义 | 例子 |
|----------|------|------|
| **三体 (Trisomy)** | 某染色体有 **3 个**拷贝（2n+1） | 唐氏综合征（21 三体） |
| **单体 (Monosomy)** | 某染色体只有 **1 个**拷贝（2n−1） | Turner 综合征 (XO) |

### 5.3 常染色体不分离——唐氏综合征

**唐氏综合征 (Down Syndrome)** 是最常见的染色体数目异常疾病，由 **21 号染色体三体 (Trisomy 21)** 引起。

| 特征 | 说明 |
|------|------|
| **病因** | 21 号染色体多一个拷贝（共 3 条） |
| **发病率** | 约 1/800（美国数据） |
| **危险因素** | **母亲年龄**——45 岁以上母亲的风险约 6% |
| **临床特征** | 特殊面容、身材矮小、心脏缺陷、智力障碍 |
...
| 核型 | 表型 | 说明 |
|------|------|------|
| **XO** | **Turner 综合征 (Turner's Syndrome)** | 女性，只有 1 条 X 染色体——唯一可存活的人类单体 |
| **XXX** | 接近正常女性 | 多一条 X，通常无明显异常 |
| **XXY** | **Klinefelter 综合征 (Klinefelter's Syndrome)** | 男性，多一条 X——睾丸发育不全、不育 |
| **XYY** | 正常或接近正常男性 | 多一条 Y，通常无明显异常 |
| **OY** | **致死** | 缺少 X 染色体，无法存活 |

> [!warning] 为什么 X 染色体异常比常染色体异常更常见？
> 女性通过 **X 染色体失活 (X Inactivation)** 机制只保留一条活跃的 X 染色体，多余的 X 染色体会被沉默为**巴氏小体 (Barr Body)**——因此额外的 X 染色体对表型的影响被缓冲了。

### 5.5 胎儿检测 (Fetal Testing)

对于可能携带遗传病或高龄产妇，可以进行胎儿检测：

| 检测方法 | 优点 | 风险 |
|----------|------|------|
| **羊膜穿刺 (Amniocentesis)** | 诊断染色体异常和其他缺陷 | 轻微感染风险、流产风险 |
| **绒毛膜取样 (Chorionic Villus Sampling, CVS)** | 诊断染色体异常和某些遗传缺陷 | 流产风险、感染风险、新生儿肢体缺陷风险 |
| **胎儿血液取样 (Fetal Blood Sampling)** | 诊断遗传/染色体异常，检查血液问题和氧水平 | 取样部位出血、感染、羊水泄漏、胎儿死亡风险 |

---

## 6. 多基因遗传与环境因素

### 6.1 多基因遗传 (Polygenic Inheritance)
...
2. **显性遗传病**：杂合子 ($Aa$) 即发病
3. **系谱分析**：利用家族图谱推断基因型和遗传方式
4. **不完全显性与共显性**：杂合子中间表型（粉花）或同时表达两种产物（AB 血型）
5. **复等位基因**：群体中存在 >2 个等位基因（如 ABO 血型）
6. **上位性**：一个基因掩盖另一个基因的表达
7. **伴性遗传**：X 连锁隐性性状在男性中更常见（如色盲、血友病）
8. **剂量补偿**：雌性失活一条 X 染色体形成巴氏小体
9. **染色体不分离**：减数分裂中染色单体未正确分开 → 三体或单体
10. **唐氏综合征**：21 三体，发病率约 1/800，风险随母亲年龄增加而升高
11. **性染色体异常**：Turner 综合征 (XO)、Klinefelter 综合征 (XXY)
12. **胎儿检测**：羊膜穿刺、绒毛膜取样、胎儿血液取样——各有风险和收益
13. **多基因与环境**：多数性状由多基因控制并受环境修饰，呈连续的正态分布

## 8. 相关笔记

- [[Mendelian Genetics|孟德尔遗传学]] - 显隐性、分离定律与自由组合定律等基础遗传规律
- [[Meiosis|减数分裂]] - 等位基因分离与非同源染色体自由组合的细胞学基础
- [[DNA Replication|DNA 复制]] — 端粒在染色体复制中的角色
- [[Genes|基因结构]] — 基因作为遗传物质的结构基础
