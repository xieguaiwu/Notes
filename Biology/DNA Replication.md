...
4. **RNA 引物酶 (RNA Primase)** 在每条 DNA 链上添加一段短的 **RNA 引物 (RNA Primer)**，为 DNA 聚合酶提供合成的 3'-OH 起点
...
graph LR
    ...
    subgraph 真核
        C["多个起点<br/>线性 DNA"] --> D["多个复制泡<br/>同时复制"]
    end
...
真核 DNA 更长，但通过**多个复制起点同时开工**来保证在 S 期内完成复制。真核的冈崎片段比原核短——约 **100-200 个核苷酸**（原核约 1,000-2,000 个核苷酸）。
...
3. **关键酶**：DNA 解旋酶、单链结合蛋白、RNA 引物酶、DNA 聚合酶、DNA 连接酶、拓扑异构酶
4. **DNA 合成方向**：始终 **5' → 3'**，需要 RNA 引物提供游离 3'-OH
5. **半不连续合成**：前导链连续、后随链不连续（冈崎片段→DNA 连接酶连接）
6. **原核 vs 真核**：原核单一起点（环状 DNA）、真核多起点（线性 DNA）
7. **端粒**：真核染色体末端的保护结构，由端粒酶维持——随分裂次数的增加端粒逐渐缩短，与细胞衰老相关
