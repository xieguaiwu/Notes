...
>命题P（谓词逻辑形式化）：$$\vdash\forall x, \neg (x)D(x) \wedge (x)E(x) $$

模态形式化：
1. 重要性：原命题所代表的是**一种世界观层面的真值赋值**，而非一个简单的合取关系
2. 方法：将表退化和演化的函数(x)D(x)和(x)E(x)改为两个可能世界下的情况
$$\text{Let G be a possible world:} \psi(\mathbf{G}) = {x_1,\ldots,x_n}$$
$$\text{Propsosition P:}\vdash\square\neg\varphi((x)D(x), \mathbf{G})\Rightarrow\varphi(\square(x)E(x),\mathbf{G})$$
