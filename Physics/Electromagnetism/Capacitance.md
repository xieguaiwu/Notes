...
> [!tip] 💡 一句话直觉
> 电容就像**水桶**：
> - 电容 $C$ = 水桶的粗细 → 越大的桶装的水（电荷）越多
> - 电压 $V$ = 水位高度 → 电压越高，储存的电荷越多
> - $Q = CV$ = 水桶里的水量 = 桶的粗细 × 水位高度
> - 更大的电容 = 能存更多电荷，但充放电也更慢
...
> [!warning] ⚠️ 容易混淆的点
> - **$C = Q/V$ 不是定义电容的原因，而是结果**：实际上是「给导体一定电荷 $Q$ 后，它的电势升到 $V$」，两者成正比，比例就是 $C$
> - **电容与 $Q$ 和 $V$ 无关**：改变 $Q$ 或 $V$ 会改变比值中的分子和分母，但比例常数 $C$ 不变——$C$ 只由几何结构决定
...
> [!question] 🤔 为什么能量是 $\frac{1}{2}$ 而不是 $QV$？
> 如果 $V$ 是常数，能量就是 $QV$。但实际情况是：从 0 开始充电，**电压随电荷增加而线性上升**（$V = q/C$）。所以平均电压只有最终电压的一半，能量是 $\frac{1}{2}QV$。这就像给气球打气——开始容易（电压低），后面越来越费力（电压高）。
...
> 
> 💡 **能量储存在电场中，不是极板上**！这个观念转变很重要——电磁波在真空中传播时，能量就是以电场和磁场的形式存在的。
