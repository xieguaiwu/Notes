...
## 9. 不同运动中物体速度
>三个相同的物块分别沿不同的路径从高度 $h$ 运动到地面。物块 A 从静止释放，竖直下落。物块 B 从静止释放，沿无摩擦斜面下滑。物块 C 以初速度 $v$ 水平抛出。哪个物块落地所需时间最长？

![[different_period_to_fall.png]]
...
### 物块 C：平抛运动
水平初速度不影响竖直方向运动。竖直方向初速度为零，加速度为 $g$，与 A 完全相同：

$$t_C = \sqrt{\frac{2h}{g}} = t_A$$

### 物块 B：斜面下滑
设斜面倾角为 $\theta$，则：
- 加速度：$a = g\sin\theta$（沿斜面向下）
- 斜面长度：$s = \dfrac{h}{\sin\theta}$

由运动学公式：$$s = \frac{1}{2}at_B^2$$
$$\frac{h}{\sin\theta} = \frac{1}{2}g\sin\theta \cdot t_B^2$$
$$t_B = \sqrt{\frac{2h}{g\sin^2\theta}}$$

### 时间比较
由于 $0 < \sin\theta < 1$，故 $\sin^2\theta < 1$：
$$t_B = \frac{t_A}{\sin\theta} > t_A = t_C$$

## 10.
>一个人对物体施加一个方向恒定、大小持续增加的力。上图显示了物体的加速度作为施加力的函数图像。解释为什么最佳拟合直线不通过原点

![[force_and_acceleration_on_graph.png]]
### 图像分析

从 $a$-$F$ 图可以看出：
- 数据点近似呈线性关系
- **拟合直线不通过原点**
- 外推至 $F = 0$ 时，$a \approx -1$ m/s²（负截距）
...
如果只有施加力 $F$ 作用，则 $a = \dfrac{F}{m}$，图像应过原点。

拟合直线不过原点且存在**负截距**，说明**存在与施加力方向相反的恒定阻力**（如摩擦力）：

$$a = \frac{F - f_k}{m} = \frac{1}{m}F - \frac{f_k}{m}$$
