...
> [!note] 特征 (characteristic)
> 使 $n \cdot 1 = 0$ 的最小正整数 $n$；若不存在，特征为 $0$。$\mathbb{Q}, \mathbb{R}, \mathbb{C}$ 的特征都是 $0$；$\mathbb{F}_p$ 的特征是 $p$。
...
- $(\mathbb{F}_p, +, \times)$ 是域
- **但** $(\mathbb{F}_p, +, \times, <)$ **不是**有序域，因为加法保序失败（$p \geq 3$）：

$$1 < 2,\quad \text{但取 } z = p-2:\; 1 + z = p-1 > 0 = 2 + z$$

加法保序要求 $1 + z < 2 + z$，实际却是 $1 + z > 2 + z$——违反。

> 更根本地，**任何有限域都不能成为有序域**（有序域必须特征 $0$ 且无穷）：有序域必有 $1 > 0$，递推得任意有限项和 $1+1+\cdots+1 > 0$；但特征 $p > 0$ 时 $p \cdot 1 = 0$，矛盾。
...
- $(\mathbb{R}, +, \times)$ 是域
- **但** $(\mathbb{R}, +, \times, <')$ **不是**有序域：加法保序要求 $1 <' -1 \implies 1 + 1 <' -1 + 1$，即 $2 <' 0$；但按 $<'$ 的定义，$2 >' 0$（因为 $\varphi(2) = 2 > 0 = \varphi(0)$）。矛盾。
...
- [[Group]] — 群的定义，域的基础构件
- [[Density and Completeness#0.7 为什么还需要域？（从序到算术）]] — Rudin 引入域公理的动机
