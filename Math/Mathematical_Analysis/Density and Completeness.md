...
Rudin 引入域的真正原因，恰恰在于其**内容的平凡性与形式上的不可或缺性**。
...
**证明**（Rudin 1.20(b)，依赖 1.20(a) 的 Archimedes 性质）：

1. 由 $y - x > 0$，取正整数 $n$ 使 $n(y-x) > 1$（把间隔放大到超过 1）；
2. 再用 1.20(a) 取 $m_1 > nx$、$m_2 > -nx$，得 $-m_2 < nx < m_1$；
3. 取整数 $m$（$-m_2 \leq m \leq m_1$）使 $m - 1 \leq nx < m$；
4. 合并得 $nx < m \leq 1 + nx < ny$，除以 $n > 0$：$x < m/n < y$。$\square$
...
Rudin 在《Principles of Mathematical Analysis》第一章引言给出一个例证。它同时展示了 $\mathbb{Q}$ 的稠密性和不完备性。
