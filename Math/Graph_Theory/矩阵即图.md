...
## 2. 任意矩阵都是二分图 (Every Matrix as a Bipartite Graph)
...
## 3. 方阵作为有向图 (Square Matrices as Directed Graphs)
...
| **上三角矩阵** $a_{ij} = 0$（当 $i > j$ 时） | 有向无环图 (DAG) — 边只能从小编号指向大编号 |
| **邻接矩阵** $A_{ij} \in \{0, 1\}$ | 无权有向图（仅表示边的存在性） |
| **随机矩阵** (列和为 1) | 每个顶点的出边权重之和为 1 — Markov 链转移图 |
| **幂零矩阵** $A^k = 0$ 对某 $k$ | 图上没有长度 $\ge k$ 的行走 (walk)（有向无环 + 严格分层） |

> [!note] 对称情形
> 当 $A$ 对称时，$A_{ij} = A_{ji}$。对应的有向图中有双向边 $(v_i, v_j)$ 和 $(v_j, v_i)$。在无向图的约定中，这两条边被视为同一条无向边。因此，**对称矩阵对应于无向图**，这是谱图理论的起点。详见 [[Adjacency Matrix & Spectrum]]。

---

## 4. 矩阵乘法与路径计数 (Matrix Multiplication as Path Counting)
...
### 4.2 矩阵幂与行走计数 (Matrix Powers and Walk Counting)
...
## 5. 秩与图结构 (Rank and the Structure of Graphs)
...
## 6. 转置与边反向 (Transpose as Reversal)
...
## 7. 谱分解与图分解 (Spectral Decomposition = Graph Decomposition)
...
> [!info] 谱与图的全局结构
> 谱不是图的完全不变量（存在非同构图有相同谱，即"同谱图"），但它携带了丰富的全局信息：连通性、二分性、团结构、扩展性质等。见 [[Laplacian & Spectral Clustering]]。
...
详见 [[Laplacian & Spectral Clustering]]。
...
- [[Adjacency Matrix & Spectrum]] — 邻接矩阵的谱性质、特征值与图结构
- [[Laplacian & Spectral Clustering]] — 拉普拉斯矩阵、Fiedler 向量、谱聚类算法
